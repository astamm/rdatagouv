---
title: "Le package R [{datagouv}](https://astamm.github.io/datagouv/index.html) : de la découverte des données à la reproductibilité"
subtitle: "Documentation technique — API data.gouv, httr2, URI, tests et mocking"
author: "Équipe datagouv"
date: 2026-08-21
lang: fr-FR
format: html
editor: source
...
[{datagouv}](https://astamm.github.io/datagouv/index.html), un client de l'API publique de la plateforme française
...
# Le package R [{datagouv}](https://astamm.github.io/datagouv/index.html) : intention et objectif principal

Le package [{datagouv}](https://astamm.github.io/datagouv/index.html) est un client R pour l'API publique de
...
```{r}
#| label: tbl-workflow
#| tbl-cap: "**Flux de travail principal dans le package [{datagouv}](https://astamm.github.io/datagouv/index.html).** Chaque étape est assurée par une fonction dédiée, plus quelques fonctions utilitaires exposées."
#| message: false
#| warning: false
#| echo: false
data.frame(
...
Les workflows typiques sont résumés dans la vignette [*Getting started*](https://astamm.github.io/datagouv/articles/datagouv.html) et dans le [README](https://astamm.github.io/datagouv/index.html) du package. @fig-workflow illustre le flux de travail principal en montrant comment les fonctions s'enchaînent pour passer de la recherche d'un jeu de données au re-téléchargement exact d'une table.

```{mermaid}
%%| label: fig-workflow
%%| fig-cap: "**Flux de travail principal dans le package [{datagouv}](https://astamm.github.io/datagouv/index.html).** Chaque étape est assurée par une fonction dédiée, plus quelques fonctions utilitaires exposées."
flowchart TD
...
Dans `datagouv`, le téléchargement (`dg_pull_dataset`), le re-téléchargement
...
Dans `datagouv`, elle n'est **pas utilisée** par l'implémentation actuelle.
...
un package client comme `datagouv` ne doit pas laisser les détails HTTP fuiter
...
Dans `datagouv`, ce helper est `req_data_gouv()` (`.env` interne,
...
exactement comme `datagouv` mocke `http_perform()` dans ses tests. C'est la
...
construire <- function() {
  request("https://www.data.gouv.fr/api/2/datasets/search/") |>
    ...
    req_user_agent("datagouv exemple") |>
...
# Les URI : définition, problème résolu et usage dans `datagouv`
...
## Comment `datagouv` construit une URI par table
...
Les tests unitaires de `datagouv` les testent en **réseau libre** (network-free)
...
## Les deux niveaux de couture dans `datagouv`
...
## Pourquoi et quand on mocke dans `datagouv`
...
# L'API conçue du package `datagouv` : les 7 exports
...
Le package `datagouv` illustre une conception aboutie de client pour une API
