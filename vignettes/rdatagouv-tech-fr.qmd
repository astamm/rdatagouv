...
CSV.GZ, TSV, TXT) avec un unique appel à [`vroom::vroom()`](https://vroom.r-lib.org), qui **devine**
...
[`vroom::cols()`](https://vroom.r-lib.org/reference/cols.html) ; le package ne dépend pas de [{readr}](https://readr.tidyverse.org). Le typage manuel **l'emporte toujours** en cas de collision avec l'inférence ou le profil.
...
de [`vroom::cols()`](https://vroom.r-lib.org/reference/cols.html) à partir du **profil `csv-detective`** publié par la plateforme [data.gouv](https://www.data.gouv.fr/) elle-même
...
que [`vroom::vroom()`](https://vroom.r-lib.org) l'infère plutôt que de figer un type peu fiable. Un `col_types` explicite
gagne toujours en cas de collision.

Passer `use_tabular_types = FALSE` désactive entièrement cette amorce et laisse
tout à l'inférence de [`vroom::vroom()`](https://vroom.r-lib.org).
...
peut contenir un mélange de types, et l'inférence de [`vroom::vroom()`](https://vroom.r-lib.org) peut se tromper. Les
avertissements par cellule de [`vroom::vroom()`](https://vroom.r-lib.org) (qui se déclenchent dès qu'il s'engage sur un
...
exceptions comme `2021-7-01` ou `2024-11-5`. [`vroom::vroom()`](https://vroom.r-lib.org) voit des
