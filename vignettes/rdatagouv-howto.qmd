...
# Le parsing des colonnes : types, profil et problèmes {#sec-parsing}

`dg_pull_dataset()` et `dg_refetch()` lisent les fichiers délimités (CSV,
CSV.GZ, TSV, TXT) avec une unique appel [`vroom::vroom()`](https://vroom.r-lib.org), qui **devine**
...
vroom pour les colonnes nommées. Les colonnes non nommées gardent leur
inférence. Les abréviations reconnues sont `"character"`, `"double"`/`"numeric"`,
...
Le mécanisme est translaté en interne par `col_types_to_spec()` en une spec
[`vroom::cols()`](https://vroom.r-lib.org/reference/cols.html) ; le package ne dépend pas de {readr}. Les colonnes ainsi
forcées **gagnent toujours** en cas de collision avec l'inférence ou le profil.
...
de vroom à partir du **profil `csv-detective`** publié par data.gouv lui-même
...
chaque ressource utilise son propre profil, pas une devinette sur le premier
candidat.
...
peut contenir un mélange de types, et l'inférence de vroom peut se tromper. Les
avertissements par cellule de vroom (qui se déclenchent dès qu'il s'engage sur un
collecteur et rencontre une valeur qu'il ne peut convertir) sont **étouffés**
avec `withCallingHandlers`, et les problèmes sous-jacents sont capturés avec
[`vroom::problems()`](https://vroom.r-lib.org/reference/problems.html) puis attachés à la table comme attribut
`rdatagouv_problems`. C'est délibéré : `vroom::problems()` opère sur l'objet
*vroom* lui-même, mais la conversion en tibble par `format_tibble()` retire la
classe vroom — l'avertissement par cellule aurait été une impasse pour
l'utilisateur final. L'attribut, lui, survit à `tibble::as_tibble()` et est lu par
...
### Le déclencheur fréquent : les dates ISO majoritairement rembourrées

Un cas réel courant est une colonne de dates telle que `2021-07-01`, **rembourrée**
(chiffres sur deux positions) pour presque toutes les lignes, avec quelques
traînard·es non rembourré·es comme `2021-7-01` ou `2024-11-5`. vroom voit des
dates ISO majoritairement rembourrées, s'engage sur un collecteur `Date`, et
signale chaque traînard·e comme un problème de parsing — les valeurs fautives
deviennent `NA`. C'est une **qualité de donnée** à corriger, pas un bug du
téléchargement. Deux façons de la corriger :

1. **Forcer `"character"`** pour ne rien perdre — aucune valeur ne devient `NA`,
   et vous pouvez parser les dates vous-même ensuite.
2. **Forcer `"Date"`** et accepter que les traînard·es deviennent `NA`, ce qui est
...
repérer la colonne fautive dans `col`, choisir une entrée `col_types` alignée sur
...
forcer `"double"` le transforme en `NA`. Dans les deux cas, vous avancez avec une
