...
- New technical vignette, *Documentation technique du package rdatagouv* —
  ...
  line with the current exports. The superseded `TOPIC-SUPPORT-SKETCH.md`
  decision log was removed (its `topic =`/`dg_find_topics()` work is fully
  shipped).
...
- `dg_find_datasets(schema_only = TRUE)` no longer silently returns an
  ...
  `resources = TRUE` now forces `resources = TRUE` and emits an informative
  message about the extra requests, so the filter actually runs instead of
  returning every row with `has_schema = NA`.
