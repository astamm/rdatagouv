...
configuration. Column types are inferred by vroom, but you can force
them with `col_types = c(col = "Date")` (e.g. to make a mostly- padded
ISO date column with a few non-padded stragglers read as `Date`, turning
the stragglers into `NA`). Any parsing issues are attached to the
returned table as an `rdatagouv_problems` attribute — read them with
`dg_problems(tbl)` — rather than a noisy per-cell warning.
